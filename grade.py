...
for row_cnt in range(1, 60):
    ...
    while DoAnotherRow == True:
        ...
        # Add Student Number
        df_hist['Student_Number'][curr_stud_row] = df_transcript['Unique ID'][row_cnt + sec_row_cnt]
        print(df_hist['Student_Number'][curr_stud_row])        

        # Add Course Name
        df_hist['Course Name'][curr_stud_row] = df_transcript['Course Name'][row_cnt]
        df_hist['Course Name'][curr_stud_row].strip()[-2:]
        
        # Add Course Number
        df_hist['Course Number'][curr_stud_row] = df_transcript['Course Number'][row_cnt]
        
        # EarnedCrHrs - 0.5, 1 TODO: How to calculate?
        df_hist['EarnedCrHrs'][curr_stud_row] = 0.5 #df_transcript['Course Number'][row_cnt]
        ...
        if sec_row_cnt == 0:
            if str(df_transcript['RC Column 4'][row_cnt]).strip() != "nan":
                df_hist['Grade'][curr_stud_row] = str(df_transcript['RC Column 4'][row_cnt]).strip()
        else: 
            if str(df_transcript['RC Column 8'][row_cnt]).strip() != "nan":
                df_hist['Grade'][curr_stud_row] = str(df_transcript['RC Column 8'][row_cnt]).strip()
    
        # PotentialCrHrs - 0.5, 1 TODO: How to calculate?
        df_hist['PotentialCrHrs'][curr_stud_row] = 0.5 #df_transcript['Course Number'][row_cnt]

        # Storecode - S1, T1, Y1, Q3 TODO:What is this?
        df_hist['Storecode'][curr_stud_row] =  'S1' #df_transcript['Course Number'][row_cnt]

        # Termid - 1200 TODO: What is the term id
        curr_termid =  df_transcript['Calendar Year'][row_cnt] - START_OF_CAL 
        df_hist['Termid'][curr_stud_row] = 1234

        # GPA Points - 4 TODO: What do we need to add with credit
        if pd.isnull(df_hist['Grade'][curr_stud_row]) == False:
            df_hist['GPA Points'][curr_stud_row] = let_to_GPA[df_hist['Grade'][curr_stud_row]]
        ...
        # Grade_Level - 10
        df_hist['Grade_Level'][curr_stud_row] = df_transcript['Grade Level'][row_cnt]

        # Credit Type - Units, or MA
        df_hist['Credit Type'][curr_stud_row] = 'Units' #df_transcript['Grade Level'][row_cnt]

        # Teacher Name - Mary Smith
        df_hist['Teacher Name'][curr_stud_row] = df_transcript['Staff Name'][row_cnt]

        # Schoolid - 100 TODO: What is the school id?
        df_hist['Schoolid'][curr_stud_row] =  5566 #df_transcript['Course Number'][row_cnt]

        # ExcludeFromGPA - 1 or 0 TODO: Is this always 0?  Same for two below
        df_hist['ExcludeFromGPA'][curr_stud_row] = 0 #df_transcript['Course Number'][row_cnt]
        
        # ExcludeFromClassRank - 1 or 0
        df_hist['ExcludeFromClassRank'][curr_stud_row] = 0 #df_transcript['Course Number'][row_cnt]
        
        # ExcludeFromHonorRoll - 1 or 0
        df_hist['ExcludeFromHonorRoll'][curr_stud_row] = 0 #df_transcript['Course Number'][row_cnt]

        # Need to check for courses that need two rows.  If this does, set 
        # Second Row Counter to 2 else
        blerh =  df_hist['Course Name'][curr_stud_row].strip()[-2:]
        print('blerh = ' + blerh)
